...
const notificationChannels = [
  { id: 'whatsapp', label: 'WhatsApp', suffix: '' },
  { id: 'sms', label: 'SMS', suffix: 'Sms' },
  { id: 'email', label: 'Email', suffix: 'Email' }
];

const notificationTemplateCatalog = [
  {
    key: 'bookingReceived',
    label: 'Booking received',
    defaultText: 'Hello {{customerName}}, your booking {{bookingId}} has been received by {{companyName}}. Contact: {{phone}}'
  },
  {
    key: 'bookingConfirmed',
    label: 'Booking confirmed',
    defaultText: 'Hello {{customerName}}, your booking {{bookingId}} is confirmed. WhatsApp: {{whatsapp}}'
  },
  {
    key: 'technicianAssigned',
    label: 'Technician assigned',
    defaultText: 'Hello {{customerName}}, a technician has been assigned for booking {{bookingId}}.'
  },
  {
    key: 'workOrderCompleted',
    label: 'Work order completed',
    defaultText: 'Hello {{customerName}}, your service work order is completed by {{companyName}}.'
  },
  {
    key: 'invoiceGenerated',
    label: 'Invoice generated',
    defaultText: 'Hello {{customerName}}, invoice {{invoiceNumber}} for {{amount}} is ready.'
  },
  {
    key: 'amcReminder',
    label: 'AMC reminder',
    defaultText: 'Hello {{customerName}}, this is an AMC reminder from {{companyName}}.'
  },
  {
    key: 'paymentReceived',
    label: 'Payment received',
    defaultText: 'Hello {{customerName}}, payment of {{amount}} has been received. Thank you.'
  },
  {
    key: 'quotationGenerated',
    label: 'Quotation generated',
    defaultText: 'Hello {{customerName}}, quotation {{quotationNumber}} for {{amount}} has been generated by {{companyName}}.'
  },
  {
    key: 'quotationApproved',
    label: 'Quotation approved',
    defaultText: 'Hello {{customerName}}, quotation {{quotationNumber}} has been approved. We will proceed with your service.'
  },
  {
    key: 'quotationDenied',
    label: 'Quotation denied',
    defaultText: 'Hello {{customerName}}, quotation {{quotationNumber}} was marked as denied. Contact {{companyName}} for assistance.'
  },
  {
    key: 'partApprovalRequest',
    label: 'Part approval request',
    defaultText: 'Hello {{customerName}}, approval is required for parts on booking {{bookingId}}. Estimated amount: {{amount}}.'
  },
  {
    key: 'technicianOnTheWay',
    label: 'Technician on the way',
    defaultText: 'Hello {{customerName}}, {{technicianName}} is on the way for your booking {{bookingId}}.'
  },
  {
    key: 'serviceCompletedPdfAttached',
    label: 'Service completed with PDF attached',
    defaultText: 'Hello {{customerName}}, your service is completed. The service PDF for {{bookingId}} is attached.'
  },
  {
    key: 'paymentDue',
    label: 'Payment due',
    defaultText: 'Hello {{customerName}}, payment of {{amount}} for invoice {{invoiceNumber}} is due on {{dueDate}}.'
  },
  {
    key: 'paymentOverdue',
    label: 'Payment overdue',
    defaultText: 'Hello {{customerName}}, payment of {{amount}} for invoice {{invoiceNumber}} is overdue. Please contact {{companyName}}.'
  },
  {
    key: 'amcExpiry30Days',
    label: 'AMC expiry 30 days',
    defaultText: 'Hello {{customerName}}, your AMC {{amcId}} expires in 30 days on {{amcExpiryDate}}.'
  },
  {
    key: 'amcExpiry15Days',
    label: 'AMC expiry 15 days',
    defaultText: 'Hello {{customerName}}, your AMC {{amcId}} expires in 15 days on {{amcExpiryDate}}.'
  },
  {
    key: 'amcExpiry7Days',
    label: 'AMC expiry 7 days',
    defaultText: 'Hello {{customerName}}, your AMC {{amcId}} expires in 7 days on {{amcExpiryDate}}.'
  }
];

function notificationFieldKey(templateKey, channelId) {
  const channel = notificationChannels.find((item) => item.id === channelId) || notificationChannels[0];
  return `${templateKey}${channel.suffix}`;
}

function notificationEnabledKey(templateKey) {
  return `${templateKey}Enabled`;
}

function buildNotificationTemplateDefaults() {
  return notificationTemplateCatalog.reduce((result, template) => {
    notificationChannels.forEach((channel) => {
      result[notificationFieldKey(template.key, channel.id)] = template.defaultText;
    });
    result[notificationEnabledKey(template.key)] = true;
    return result;
  }, {});
}
...
const notificationVariableGroups = [
  ...
  {
    label: 'Booking',
    variables: ['{{bookingId}}', '{{bookingDate}}', '{{serviceType}}', '{{deviceType}}', '{{technicianName}}', '{{workOrderId}}']
  },
  {
    label: 'Billing',
    variables: ['{{quotationNumber}}', '{{invoiceNumber}}', '{{amount}}', '{{dueDate}}', '{{paymentLink}}']
  },
  {
    label: 'Company',
    variables: ['{{companyName}}', '{{phone}}', '{{whatsapp}}', '{{email}}', '{{address}}']
  },
  {
    label: 'AMC',
    variables: ['{{amcId}}', '{{amcPlan}}', '{{amcExpiryDate}}', '{{amcVisitDate}}', '{{amcDaysLeft}}']
  }
];

const notificationVariables = notificationVariableGroups.flatMap((group) => group.variables);
const notificationVariableSet = new Set(notificationVariables);

const notificationPreviewValues = {
  ...
  '{{amount}}': 'Rs. 4,500',
  '{{dueDate}}': '10 Jun 2026',
  '{{paymentLink}}': 'https://pay.example.com/inv-2048',
...
function BusinessSettingsFrame({
  ...
  return (
    <form className="grid gap-5" onSubmit={save}>
      ...
      {children({ form, setForm, setPath, canEdit, saving, saved, defaults, dirty, resetForm, resetDefaults })}
...
function normalizeNotificationVariable(token = '') {
  const name = String(token || '').replace(/[{}]/g, '').trim();
  return name ? `{{${name}}}` : '';
}

function invalidVariablesForText(value = '') {
  const matches = String(value || '').match(/{{[^{}]+}}/g) || [];
  return Array.from(new Set(matches
    .map((item) => normalizeNotificationVariable(item))
    .filter((item) => item && !notificationVariableSet.has(item))));
}

function invalidVariablesForTemplate(form = {}, templateKey = '') {
  return Array.from(new Set(notificationChannels.flatMap((channel) => invalidVariablesForText(form[notificationFieldKey(templateKey, channel.id)] || ''))));
}

function compactNotificationPreview(value = '') {
  const text = String(value || '').replace(/\s+/g, ' ').trim();
  if (!text) return 'No template text saved yet.';
  return text.length > 132 ? `${text.slice(0, 129)}...` : text;
}

function renderNotificationPreview(value = '') {
  const text = String(value || '').trim();
  if (!text) return 'No preview text available.';
  return text.replace(/{{\s*([^{}]+?)\s*}}/g, (match, name) => notificationPreviewValues[`{{${String(name).trim()}}}`] || match);
}

function NotificationPreviewModal({ template, channel, value, invalidVariables, onClose }) {
  if (!template || !channel) return null;
  return (
    <div className="fixed inset-0 z-[90] grid place-items-center bg-black/55 p-4" role="dialog" aria-modal="true" aria-labelledby="notification-preview-title">
      <div className="surface admin-modal notification-preview-modal w-full max-w-xl p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-wide text-[var(--brand)]">{channel.label} Preview</p>
            <h2 id="notification-preview-title" className="mt-1 text-xl font-black">{template.label}</h2>
          </div>
          <button type="button" className="icon-button h-9 w-9" onClick={onClose} aria-label="Close notification preview">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="notification-preview-copy mt-4">
          {renderNotificationPreview(value)}
        </div>
        <div className="mt-5 flex justify-end">
...
function NotificationVariablesPanel({ disabled, onInsert }) {
  return (
    <aside className="surface admin-control-card notification-variables-panel p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-black">Variables</h3>
          <p className="mt-1 text-xs font-bold uppercase tracking-wide text-sky-200">{notificationVariables.length} available</p>
        </div>
        <div className="admin-control-icon h-10 w-10"><Hash className="h-4 w-4" /></div>
      </div>
      <div className="mt-4 grid gap-4">
        {notificationVariableGroups.map((group) => (
          <div key={group.label} className="notification-variable-group">
            <h4>{group.label}</h4>
            <div className="mt-2 flex flex-wrap gap-2">
              {group.variables.map((item) => (
                <button key={item} type="button" className="notification-variable-chip" disabled={disabled} onClick={() => onInsert(item)}>
                  {item}
                </button>
              ))}
            </div>
...
function NotificationTemplatesSection({ onDirtyChange = null }) {
  ...
  return (
    <BusinessSettingsFrame
      section="notificationTemplates"
      tabId="notificationTemplates"
      title="Notification Templates"
      icon={MessageSquareText}
      description="Save reusable message templates. No WhatsApp/SMS/email is sent unless a provider integration exists."
      onDirtyChange={onDirtyChange}
      resetLabel="Discard"
      showResetDefaults
      saveFirst
    >
      {({ form, setForm, setPath, canEdit, saving, defaults }) => {
        const activeTemplate = notificationTemplateCatalog.find((item) => item.key === activeTemplateKey) || null;
        const activeChannelConfig = notificationChannels.find((item) => item.id === activeChannel) || notificationChannels[0];
        const activeFieldKey = activeTemplate ? notificationFieldKey(activeTemplate.key, activeChannelConfig.id) : '';
        const activeValue = activeFieldKey ? String(form[activeFieldKey] || '') : '';
        const activeInvalidVariables = invalidVariablesForText(activeValue);
        ...
        function updateActiveTemplateValue(value) {
          if (!activeFieldKey) return;
          setPath(activeFieldKey, value);
        }

        function insertVariable(variable) {
          if (!activeTemplate || !activeFieldKey) {
            push('Open a template editor before inserting a variable.', 'info');
            return;
          }
          if (!canEdit || saving) return;
          const currentValue = String(form[activeFieldKey] || '');
          ...
          setPath(activeFieldKey, nextValue);
        ...
        function resetTemplate(templateKey) {
          const template = notificationTemplateCatalog.find((item) => item.key === templateKey);
          setForm((current) => {
            const next = clonePlain(current);
            notificationChannels.forEach((channel) => {
              const field = notificationFieldKey(templateKey, channel.id);
              next[field] = defaults[field] || '';
            });
            next[notificationEnabledKey(templateKey)] = defaults[notificationEnabledKey(templateKey)] !== false;
            return next;
          });
          if (template) push(`${template.label} reset to defaults`, 'info');
        }

        function previewTemplate(template, channelId = activeChannel) {
          const channel = notificationChannels.find((item) => item.id === channelId) || notificationChannels[0];
          const value = form[notificationFieldKey(template.key, channel.id)] || '';
          setPreviewState({
            template,
            channel,
            value,
            invalidVariables: invalidVariablesForText(value)
          });
        }

        function sendTest(template, channelId = activeChannel) {
          const channel = notificationChannels.find((item) => item.id === channelId) || notificationChannels[0];
          push(`${channel.label} test sending is not connected yet.`, 'info');
          openEditor(template.key, channel.id);
        }

        return (
          <div className="notification-template-shell" data-notification-templates-root>
            <div className="notification-template-main">
              <section className="notification-integration-row" aria-label="Notification integration status">
                {notificationChannels.map((channel) => {
                  ...
                  return (
                    <div key={channel.id} className={`notification-integration-item ${connected ? 'is-connected' : 'is-disconnected'}`}>
                      <div>
              ...
              {activeTemplate ? (
                <section className="surface admin-control-card notification-editor-panel p-5">
                  <div className="notification-editor-header">
                    <div className="min-w-0">
                      ...
                      <h3 className="mt-1 text-xl font-black">{activeTemplate.label}</h3>
                    </div>
                    <button type="button" className="icon-button h-9 w-9" onClick={() => setActiveTemplateKey(null)} aria-label="Close template editor">
                      <X className="h-5 w-5" />
                    </button>
                  </div>
                  <div className="notification-channel-tabs" role="tablist" aria-label="Notification channels">
                    {notificationChannels.map((channel) => (
                      <button
                        key={channel.id}
                        type="button"
                        className={`notification-channel-tab ${activeChannel === channel.id ? 'is-active' : ''}`}
                  ...
                  {activeInvalidVariables.length ? (
                    <div className="notification-warning">
                      <AlertTriangle className="h-4 w-4" />
                      <span>Invalid variables: {activeInvalidVariables.join(', ')}</span>
                    </div>
                  ) : null}
                  <textarea
                    ref={textareaRef}
                    className="input notification-editor-textarea"
                    value={activeValue}
                    disabled={!canEdit || saving}
                    onChange={(event) => updateActiveTemplateValue(event.target.value)}
                  />
                  <div className="notification-editor-footer">
                    <span>{activeValue.length}/1500 characters</span>
                    <div className="notification-editor-actions">
                      <button type="button" className="btn btn-secondary admin-table-button" onClick={() => previewTemplate(activeTemplate, activeChannelConfig.id)}>
                        <Eye className="h-4 w-4" />
                        Preview
                      </button>
                      <button type="button" className="btn btn-secondary admin-table-button" onClick={() => sendTest(activeTemplate, activeChannelConfig.id)}>
                        <Send className="h-4 w-4" />
                        Send Test
                      </button>
                      <button type="button" className="btn btn-secondary admin-table-button" disabled={!canEdit || saving} onClick={() => resetTemplate(activeTemplate.key)}>
                        <RotateCcw className="h-4 w-4" />
                        Reset
                      </button>
                    </div>
              ...
              <section className="notification-template-grid">
                {notificationTemplateCatalog.map((template) => {
                  const enabled = form[notificationEnabledKey(template.key)] !== false;
                  const previewValue = notificationChannels
                    .map((channel) => String(form[notificationFieldKey(template.key, channel.id)] || '').trim())
                    .find(Boolean) || '';
                  const invalidVariables = invalidVariablesForTemplate(form, template.key);
                  return (
                    <article key={template.key} className={`surface admin-control-card notification-template-card ${activeTemplateKey === template.key ? 'is-active' : ''}`}>
                      <div className="notification-template-card-header">
                        <h3>{template.label}</h3>
                        <button
                          type="button"
                          className={`notification-toggle ${enabled ? 'is-on' : 'is-off'}`}
                          disabled={!canEdit || saving}
                          aria-pressed={enabled}
                          onClick={() => setPath(notificationEnabledKey(template.key), !enabled)}
                        >
                          <span aria-hidden="true" />
                          <strong>{enabled ? 'Enabled' : 'Disabled'}</strong>
                        </button>
                      </div>
                      <p className="notification-template-preview">{compactNotificationPreview(previewValue)}</p>
                      <div className="notification-channel-chips">
                        {notificationChannels.map((channel) => (
                          <button
                            key={channel.id}
                            type="button"
                            className={`notification-channel-chip channel-${channel.id}`}
                            onClick={() => openEditor(template.key, channel.id)}
                          >
                            {channel.label}
                          </button>
                        ))}
                      ...
                      {invalidVariables.length ? (
                        <div className="notification-card-warning">
                          ...
                          <span>{invalidVariables.length} invalid variable{invalidVariables.length === 1 ? '' : 's'}</span>
                        </div>
                      ) : null}
                      <div className="notification-card-actions">
                        <button type="button" className="btn btn-secondary admin-table-button" onClick={() => openEditor(template.key)}>
                          <Edit3 className="h-4 w-4" />
                          Edit
                        </button>
                        <button type="button" className="btn btn-secondary admin-table-button" onClick={() => previewTemplate(template)}>
                          <Eye className="h-4 w-4" />
                          Preview
                        </button>
                        <button type="button" className="btn btn-secondary admin-table-button" onClick={() => sendTest(template)}>
                          <Send className="h-4 w-4" />
                          Send Test
                        </button>
                        <button type="button" className="btn btn-secondary admin-table-button" disabled={!canEdit || saving} onClick={() => resetTemplate(template.key)}>
                          <RotateCcw className="h-4 w-4" />
                          Reset
                        </button>
                      </div>
            ...
            <NotificationVariablesPanel disabled={!canEdit || saving || !activeTemplate} onInsert={insertVariable} />
            ...
            {previewState ? (
              <NotificationPreviewModal
                ...
                invalidVariables={previewState.invalidVariables}
