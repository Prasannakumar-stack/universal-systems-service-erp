...
export function workOrderPdfTotal(workOrder) {
  if (workOrder?.invoiceId?.total != null) return Number(workOrder.invoiceId.total);
  if (workOrder?.amcContractId?.invoiceId?.total != null) return Number(workOrder.amcContractId.invoiceId.total);
  if (workOrder?.amcContractId?.contractValue != null) return Number(workOrder.amcContractId.contractValue);
  const partsTotal = (workOrder?.partsUsed || []).reduce((sum, part) => sum + Number(part.total || 0), 0);
  return partsTotal + Number(workOrder?.serviceCharge || 0);
}
