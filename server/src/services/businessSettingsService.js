...
function notificationFieldKey(templateKey, channelId) {
  const channel = NOTIFICATION_CHANNELS.find((item) => item.id === channelId) || NOTIFICATION_CHANNELS[0];
...
function buildNotificationTemplateDefaults() {
  return NOTIFICATION_TEMPLATE_CATALOG.reduce((result, template) => {
...
function sanitizeNotificationTemplates(payload = {}) {
  ...
  return NOTIFICATION_TEMPLATE_CATALOG.reduce((result, template) => {
    const whatsappKey = notificationFieldKey(template.key, 'whatsapp');
    const whatsappText = cleanText(base[whatsappKey], defaultBusinessSettings.notificationTemplates[whatsappKey], 1500);
    result[whatsappKey] = whatsappText;
    result[notificationFieldKey(template.key, 'sms')] = cleanText(
      base[notificationFieldKey(template.key, 'sms')],
      whatsappText,
      1500
    );
    result[notificationFieldKey(template.key, 'email')] = cleanText(
      base[notificationFieldKey(template.key, 'email')],
      whatsappText,
      1500
    );
    result[notificationEnabledKey(template.key)] = base[notificationEnabledKey(template.key)] !== false;
    return result;
  }, {});
}
